...
import { Camera, Check, ClipboardCheck, History, Home, ListChecks, Menu, Moon } from "lucide-react";
import { Badge, Card, Page, Stat } from "@/components/ui";
import { dateLabel, locationLabelForEmployee, requireUser } from "@/lib/orisus";
...
export default async function EmployeeWorkspace({ params, searchParams }: { params: { slug?: string[] }; searchParams?: { tab?: string } }) {
  ...
  const taskTargets = section === "tasks" && slug[1] ? await getDelegationTargets(user) : [];
  const checklistDetail = ["checklists", "abendcheck"].includes(section) && slug[1] ? await getChecklistDetail(slug[1], user) : null;
  const eveningChecks = checks.filter(isEveningCheck);
  const regularChecks = checks.filter((check) => !isEveningCheck(check));
  const open = tasks.filter((task) => task.status !== "completed").length;
  const done = tasks.filter((task) => task.status === "completed").length;
  const overdue = tasks.filter((task) => task.status === "overdue").length;
  const tab = searchParams?.tab || "today";
  return <Page title={sectionTitle(section, user.first_name)} subtitle={`Standort: ${locationLabelForEmployee(user)}`}><div className="pb-24"><EmployeeNav />{section === "dashboard" && <><div className="grid gap-3 sm:grid-cols-3"><Stat label="Offen" value={open} tone="orange" /><Stat label="Erledigt" value={done} tone="green" /><Stat label="Überfällig" value={overdue} tone="red" /></div><div className="mt-6 grid gap-5 lg:grid-cols-2"><TaskList tasks={tasks.filter((task) => task.status !== "completed").slice(0, 6)} title="Heute offen" empty="Keine offenen Aufgaben" /><ChecklistList checks={regularChecks.filter((check) => check.status !== "completed")} title="Standort-Checklisten heute" empty="Keine Checklisten heute" /></div></>}{section === "tasks" && slug[1] ? <TaskDetail id={slug[1]} tasks={tasks} targets={taskTargets} /> : null}{section === "tasks" && !slug[1] ? <TaskTabs tasks={tasks} delegations={delegationRows} active={tab} /> : null}{section === "checklists" && slug[1] ? <ChecklistDetail detail={checklistDetail} returnTo={`/app/checklists/${slug[1]}`} completeReturnTo="/app/checklists" /> : null}{section === "checklists" && !slug[1] ? <ChecklistList checks={regularChecks} title="Standort-Checklisten" empty="Keine Checklisten heute" /> : null}{section === "abendcheck" && slug[1] ? <ChecklistDetail detail={checklistDetail} returnTo={`/app/abendcheck/${slug[1]}`} completeReturnTo="/app/abendcheck" /> : null}{section === "abendcheck" && !slug[1] ? <ChecklistList checks={eveningChecks} title="Abendcheck" empty="Keine Abendchecks heute" basePath="/app/abendcheck" /> : null}{section === "history" ? <HistoryView tasks={tasks} /> : null}{section === "delegations" ? <DelegationsView delegations={delegationRows} /> : null}{section === "faq" ? <FaqView /> : null}{section === "more" ? <MoreView user={user.display_name} /> : null}</div><BottomNav /></Page>;
}

function sectionTitle(section: string, name: string) {
  return ({ dashboard: `Hallo ${name}`, tasks: "Meine Aufgaben", checklists: "Standort-Checklisten", abendcheck: "Abendcheck", history: "Meine Historie", delegations: "Vertretungen", faq: "Hilfe & FAQ", more: "Mehr" } as Record<string, string>)[section] || "ORISUS";
}

function isEveningCheck(check: { name?: string }) {
  return String(check.name || "").toLowerCase().includes("abendcheck");
}
...
function TaskDetail({ id, tasks, targets }: { id: string; tasks: any[]; targets: any[] }) {
...
function EmployeeNav() {
  return <div className="mb-5 hidden gap-2 overflow-x-auto rounded-xl bg-white p-2 shadow-card sm:flex">{[["/app", "Übersicht"], ["/app/tasks", "Aufgaben"], ["/app/checklists", "Checklisten"], ["/app/abendcheck", "Abendcheck"], ["/app/history", "Historie"], ["/app/more", "Mehr"]].map(([href, label]) => <Link key={href} href={href} className="rounded-lg px-3 py-2 text-sm font-semibold text-navy hover:bg-canvas">{label}</Link>)}</div>;
}

function BottomNav() {
  const links = [["/app", "Übersicht", Home], ["/app/tasks", "Aufgaben", ClipboardCheck], ["/app/checklists", "Listen", ListChecks], ["/app/abendcheck", "Abend", Moon], ["/app/history", "Historie", History], ["/app/more", "Mehr", Menu]] as const;
  return <nav className="fixed inset-x-0 bottom-0 z-20 border-t border-slate-200 bg-white px-2 py-2 sm:hidden"><div className="grid grid-cols-6 gap-1">{links.map(([href, label, Icon]) => <Link key={href} href={href} className="flex flex-col items-center gap-1 rounded-xl px-1 py-2 text-[11px] font-medium text-slate-600"><Icon className="h-5 w-5 text-navy" />{label}</Link>)}</div></nav>;
}
