...
export async function POST(request: Request, { params }: { params: { checklistId: string; itemId: string } }) {
  const user = await requireUser("employee");
  const form = await request.formData();
  const db = supabaseAdmin();
  if (!db) return NextResponse.redirect(new URL(`/app/checklists/${params.checklistId}`, request.url));
  ...
  if (!item) return NextResponse.redirect(new URL(`/app/checklists/${params.checklistId}`, request.url));
  ...
  if (item.proof_type === "text" && !comment) {
    return NextResponse.redirect(new URL(`/app/checklists/${params.checklistId}?error=proof`, request.url));
  }

  if (item.proof_type === "photo") {
    if (!(photo instanceof File) || photo.size === 0) return NextResponse.redirect(new URL(`/app/checklists/${params.checklistId}?error=proof`, request.url));
    ...
    if (error) return NextResponse.redirect(new URL(`/app/checklists/${params.checklistId}?error=save`, request.url));
    photoUrl = db.storage.from("task-proofs").getPublicUrl(path).data.publicUrl;
  }

  await db.from("checklist_item_completions").upsert({
    checklist_occurrence_id: params.checklistId,
  ...
  await db.from("checklist_occurrences").update({ status: "in_progress" }).eq("id", params.checklistId).neq("status", "completed");
  return NextResponse.redirect(new URL(`/app/checklists/${params.checklistId}`, request.url));
}
