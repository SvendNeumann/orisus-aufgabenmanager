...
export async function POST(request: Request, { params }: { params: { id: string; itemId: string } }) {
  ...
  if (item.proof_type === "text" && !comment) {
    return NextResponse.redirect(new URL(`/app/checklists/${params.id}?error=proof`, request.url));
  }

  if (item.proof_type === "photo") {
    if (!(photo instanceof File) || photo.size === 0) return NextResponse.redirect(new URL(`/app/checklists/${params.id}?error=proof`, request.url));
    ...
    if (error) return NextResponse.redirect(new URL(`/app/checklists/${params.id}?error=save`, request.url));
