...
function Tasks() {
  return <><div className="mb-4 grid gap-2 sm:grid-cols-4">{["Standort", "Mitarbeiter", "Bereich", "Status"].map((filter) => <input key={filter} placeholder={filter} className="h-11 rounded-xl border border-slate-200 px-3 focus-ring" />)}</div><div className="space-y-3">{tasks.map((task) => <Card key={task.id} className="flex items-center justify-between"><div><p className="font-bold text-navy">{task.title}</p><p className="text-sm text-slate-500">{task.area} · fällig {task.due_time}</p></div><Badge status={task.status} /></Card>)}</div></>;
}
