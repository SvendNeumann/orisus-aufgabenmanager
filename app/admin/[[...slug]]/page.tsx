...
import { checklists, dateLabel, delegations, employees, locations, requireUser, tasks } from "@/lib/orisus";
...
export default async function AdminWorkspace({ params }: { params: { slug?: string[] } }) {
  ...
  return <Page title={title(section)} subtitle="Standortübergreifende Kontrolle für ORISUS."><AdminNav />{section === "dashboard" ? <><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6"><Stat label="Standorte" value={locations.length} /><Stat label="Mitarbeiter" value={employees.length} /><Stat label="Offen" value={open} tone="orange" /><Stat label="Erledigt" value={done} tone="green" /><Stat label="Überfällig" value={overdue} tone="red" /><Stat label="Checklisten" value={checklists.length} /></div><LocationCards /></> : null}{section === "locations" && id ? <LocationDetail id={id} /> : null}{section === "locations" && !id ? <LocationCards /> : null}{section === "employees" ? <Employees /> : null}{section === "tasks" ? <Tasks /> : null}{section === "checklists" ? <Checklists /> : null}{section === "delegations" ? <Delegations /> : null}{section === "history" ? <History /> : null}{section === "settings" ? <Settings /> : null}</Page>;
}
...
function LocationDetail({ id }: { id: string }) {
  ...
  const locationTasks = tasks.filter((item) => item.location_id === location.id);
